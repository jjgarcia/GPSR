...
void ValuePropagation::startup()
{
	...
	sentOnce = 0;
	theValue = 0;
	setTimer(REQUEST_SAMPLE, 14);//  sec timer
}
		...
		const char *source, double rssi, double lqi)
{
	...
	if (receivedData > tempThreshold ){// && !sentOnce) {
		...
		toNetworkLayer(createGenericDataPacket(receivedData, 1),0);//BROADCAST_NETWORK_ADDRESS);          //JJ: put 0 to send packet to coordinator   0);//
		sentOnce = 1;
		trace() << "fromNetworkLayer: Got the value: " << theValue;
	}
}
...
void ValuePropagation::handleSensorReading(SensorReadingMessage * rcvReading)
{
	...
	if (sensedValue > tempThreshold){//&& !sentOnce) {
		trace() << "Node["<< SELF_NETWORK_ADDRESS <<"] measured: "<< sensedValue; 
		theValue = sensedValue;
		toNetworkLayer(createGenericDataPacket(sensedValue, 1),0);// BROADCAST_NETWORK_ADDRESS);
		sentOnce = 1;
	}
}
